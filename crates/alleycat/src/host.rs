...
use crate::framing::{
    MAX_REMORA_LINK_V2_FRAME_BYTES, read_json_frame, read_json_frame_bounded, write_json_frame,
...
async fn handle_stream_v1(
    ...
    let request: Request =
        tokio::time::timeout(INITIAL_REQUEST_TIMEOUT, read_json_frame(&mut recv))
            .await
            .context("timed out waiting for initial alleycat/1 request")??;
