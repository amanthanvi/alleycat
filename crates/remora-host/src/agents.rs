...
use remora_bridge_core::session::{Session, SessionRegistry, SessionRegistryConfig};
...
use remora_claude_bridge::ClaudeBridge;
use remora_devin_bridge::DevinBridge;
use remora_droid_bridge::DroidBridge;
use remora_grok_bridge::GrokBridge;
use remora_hermes_bridge::{HermesBridge, HermesBridgeConfig};
...
/// excluded — Remora talks to it directly through Codex's app-server
...
pub struct AgentManager {
    ...
    /// Remora is proxying to an externally-started Codex app-server.
...
impl AgentManager {
    ...
    pub async fn new(config: Arc<ArcSwap<HostConfig>>) -> anyhow::Result<Self> {
        ...
        let hermes_bridge_cfg = HermesBridgeConfig {
            mode: remora_hermes_bridge::HermesMode::Auto {
        ...
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        let task =
            remora_bridge_core::start_stream_with_session(bridge, stream, session, last_seen);
    ...
    /// external Codex daemon/Desktop already owns the socket, Remora leaves it
    ...
    async fn ensure_codex_unix_running(&self) -> anyhow::Result<CodexUnixEndpoint> {
        ...
        let endpoint = match probe_codex_app_server_proxy(&bin, None, &env).await {
            ...
            Err(error) => {
                ...
                {
                    warn!(
                        "codex default control socket accepts connections but proxy websocket handshake failed; using remora-owned socket default_socket={} error={error:#}",
                        socket_path.display()
                    );
                    CodexUnixEndpoint::custom_socket(
                        bin,
                        remora_codex_control_socket_path(&socket_path),
                    )
                } else {
...
fn remora_codex_control_socket_path(default_socket_path: &Path) -> PathBuf {
    default_socket_path
        .parent()
        .map(|parent| parent.join("remora-app-server-control.sock"))
        .unwrap_or_else(|| default_socket_path.with_file_name("remora-app-server-control.sock"))
}
...
mod tests {
    ...
    fn factory_auth_accepts_v2_store() {
        ...
        let api_key_env = "REMORA_TEST_FACTORY_API_KEY_UNSET";
