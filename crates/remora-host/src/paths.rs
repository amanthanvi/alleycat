//! Cross-platform filesystem path helpers for the Remora Link daemon.
//!
...
//! | ---------- | -------------------------------------- | --------------------------- | ---------------------------------------- |
//! | config     | ~/Library/Application Support/<id>/    | $XDG_CONFIG_HOME/remora/  | %APPDATA%\Remora\remora\config\      |
//! | state      | (collapses to config_dir)              | $XDG_STATE_HOME/remora/   | %LOCALAPPDATA%\Remora\remora\data\   |
//! | logs       | ~/Library/Logs/<id>/                   | <state>/logs/               | <data_local>/logs/                       |
//! | control IPC| $TMPDIR/remora-<hash>/control.sock   | $XDG_RUNTIME_DIR or <state> | \\.\pipe\remora-control-<userhash>     |
//!
...
// host library doesn't assume which wrapper is invoking it. See `crate::App`
// for the canonical defaults used when
...
///
/// 1. `$XDG_RUNTIME_DIR/remora-<userhash>` (Linux ideal — tmpfs, per-user)
/// 2. `<state_dir>/remora-<userhash>` (Linux fallback)
/// 3. `$TMPDIR/remora-<userhash>` (macOS / BSD)
/// 4. `/tmp/remora-<userhash>` (last resort)
///
...
/// `~/Library/LaunchAgents/com.remora.link.plist` on macOS.
...
/// `~/.config/systemd/user/remora-link.service` on Linux.
#[allow(dead_code)]
...
/// `%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\remora-link.lnk`.
#[allow(dead_code)]
...
/// `~/.config/autostart/remora-link.desktop` — fallback when systemctl --user is
/// unavailable.
#[cfg(target_os = "linux")]
...
mod tests {
    ...
    fn control_socket_prefers_xdg_runtime_dir_when_set() {
        ...
        let stamp = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        let runtime = PathBuf::from(format!("/tmp/remora-rt-{}-{stamp}", std::process::id()));
    ...
    fn launchd_plist_path_label() {
        ...
        assert_eq!(p.file_name().unwrap(), "com.remora.link.plist");
    ...
    fn systemd_unit_path_endings() {
        ...
        assert!(
            p.to_string_lossy()
                .ends_with("systemd/user/remora-link.service")
        );
    }
    ...
    fn windows_startup_lnk_path_endings() {
        ...
        assert!(s.ends_with(r"Startup\remora-link.lnk"));
    }
    ...
    fn macos_log_dir_under_library_logs() {
        ...
        assert!(s.contains("Library/Logs/com.remora.remora-link"));
    }
