...
fn render_systemd_unit(
    ...
    format!(
        "[Unit]\n\
         Description=Remora bridge daemon\n\
...
mod tests {
    ...
    fn write_systemd_unit_contains_exec_start() {
        let tmp = tempdir();
        let unit = tmp.join("remora-link.service");
        let exe = PathBuf::from("/opt/remora/bin/remora-link");
        write_systemd_unit(&unit, &exe, None, None).expect("write unit");
        let body = std::fs::read_to_string(&unit).expect("read unit");
        assert!(body.contains(&format!(
            "ExecStart=/opt/remora/bin/remora-link {DAEMON_SUBCOMMAND}"
        )));
    ...
    fn write_autostart_desktop_contains_exec() {
        let tmp = tempdir();
        let desktop = tmp.join("remora-link.desktop");
        let exe = PathBuf::from("/usr/bin/remora-link");
        write_autostart_desktop(&desktop, &exe).expect("write desktop");
        let body = std::fs::read_to_string(&desktop).expect("read desktop");
        assert!(body.contains(&format!("Exec=/usr/bin/remora-link {DAEMON_SUBCOMMAND}")));
    ...
    fn systemd_unit_name_uses_actual_packaged_unit_filename() {
        let unit = PathBuf::from("/home/me/.config/systemd/user/remora-link.service");
        assert_eq!(
