...
mod tests {
    ...
    fn write_plist_renders_expected_keys() {
        let tmp = tempdir();
        let plist = tmp.join("com.remora.link.plist");
        let exe = PathBuf::from("/usr/local/bin/remora-link");
        ...
        assert!(body.contains("<string>com.remora.link</string>"));
        assert!(body.contains("<string>/usr/local/bin/remora-link</string>"));
