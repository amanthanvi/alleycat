//! Per-OS install / uninstall of an autostart entry for `remora-link serve`.
//!
//! All three platforms are first-class and **never require admin**:
//! - macOS: launchd user agent (`~/Library/LaunchAgents/com.remora.link.plist`).
//! - Linux: systemd user unit, with `~/.config/autostart/remora-link.desktop`
...
/// supplied at startup (`com.remora.link` for the shipped wrapper).
#[allow(dead_code)] // used only by the macOS service implementation
pub fn service_label() -> &'static str {
    crate::app().label
}

/// Subcommand the autostart entry invokes on the `remora-link` binary.
